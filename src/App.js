...
function SuccessPage({data,onBack,onPortal}){
  ...
  const handleOrder=async()=>{
    ...
    const{error}=await supabase.from("orders").insert({
      ...
      ...(importExtras.spezialisierung?{spezialisierung:importExtras.spezialisierung}:{}),
      ...(importExtras.gut_zu_wissen?{gut_zu_wissen:importExtras.gut_zu_wissen}:{}),
      ...(importExtras.bewertungen?.length?{bewertungen:importExtras.bewertungen,ai_generated:[...(importExtras.bewertungen.length?["bewertungen"]:[])]}:{}),
      website_ziel:null
    });

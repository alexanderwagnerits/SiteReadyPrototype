...
const INIT={firmenname:"",branche:"",brancheLabel:"",brancheCustom:"",leistungen:[],extraLeistung:"",adresse:"",plz:"",ort:"",bundesland:"",telefon:"",email:"",uid:"",oeffnungszeiten:"",oeffnungszeitenCustom:"",einsatzgebiet:"",kurzbeschreibung:"",notdienst:false,fotos:false,stil:"professional"};
      ...
      <div>
        <div style={{background:"linear-gradient(135deg,#eff4ff 0%,#f0fdf4 100%)",borderRadius:14,padding:"40px 36px",border:"1.5px solid rgba(37,99,235,.12)"}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:28}}>
            {["SEO automatisch","Impressum (ECG)","DSGVO automatisch","Google-Indexierung","Live-Vorschau","SSL inklusive","Kein Branding","Analytics","Website-Import","Branchenfotos"].map(l=><span key={l} style={{fontSize:".75rem",padding:"6px 13px",borderRadius:6,fontWeight:600,background:"#fff",color:T.green,border:"1px solid rgba(22,163,74,.15)"}}>{"\u2713"} {l}</span>)}
          </div>
...
function SuccessPage({data,onBack}){
  ...
  return(<div style={{height:"100vh",background:"#fff",fontFamily:T.font,display:"flex",flexDirection:"column",overflow:"hidden"}}><style>{css}</style>
    {/* Top bar */}
...
function Questionnaire({data,setData,onComplete,onBack}){
  const[step,setStep]=useState(0);const ref=useRef(null);const[isMobile]=useState(typeof window!=="undefined"&&window.innerWidth<768);const[showPreview,setShowPreview]=useState(false);const[showImport,setShowImport]=useState(true);const[importUrl,setImportUrl]=useState("");const[importLoading,setImportLoading]=useState(false);const simulateImport=()=>{if(!importUrl.trim())return;setImportLoading(true);setTimeout(()=>{const raw=importUrl.replace(/https?:\/\/(www\.)?/,"").split(".")[0];const nm=raw.split("-").map(w=>w.charAt(0).toUpperCase()+w.slice(1)).join(" ");setData(d=>({...d,firmenname:nm||"Muster Betrieb",branche:"elektro",brancheLabel:"Elektroinstallationen",leistungen:["Elektroinstallationen","Stoerungsbehebung & Reparatur","Smart Home Systeme"],adresse:"Hauptstrasse 12",plz:"1010",ort:"Wien",bundesland:"wien",einsatzgebiet:"Wien",telefon:"+43 1 234 56 78",email:"office@firma.at",kurzbeschreibung:"Ihr zuverlaessiger Partner in Wien. Seit Jahren fuer Privat- und Gewerbekunden.",notdienst:true,stil:"professional",oeffnungszeiten:"mo-fr-8-17"}));setImportLoading(false);setShowImport(false);},2000);};
const up=useCallback(k=>v=>setData(d=>({...d,[k]:v})),[setData]);const go=n=>{setStep(n);if(ref.current)ref.current.scrollTop=0};const pct=((step+1)/STEPS.length)*100;const selectedBranche=BRANCHEN.find(b=>b.value===data.branche);const brancheLeistungen=selectedBranche?selectedBranche.leistungen:[];const onBrancheChange=val=>{const b=BRANCHEN.find(x=>x.value===val);setData(d=>({...d,branche:val,brancheLabel:b?b.label:"",brancheCustom:"",stil:b?b.stil:d.stil,leistungen:[],extraLeistung:""}))};
  ...
  const formPanel=(<div style={{display:"flex",flexDirection:"column",background:T.bg,borderRight:isMobile?"none":`1px solid ${T.bg3}`,height:isMobile?"auto":"100%",minHeight:isMobile?"100vh":"auto",fontFamily:T.font}}>
    ...
    <div ref={ref} style={{flex:1,overflowY:"auto",padding:"20px 24px"}}>{showImport?(<div><div style={{display:"inline-flex",alignItems:"center",gap:6,background:T.accentLight,color:T.accent,padding:"5px 14px",borderRadius:100,fontSize:".65rem",fontWeight:700,letterSpacing:".1em",textTransform:"uppercase",marginBottom:18}}>Optionaler Vorschritt</div><h2 style={{fontSize:"1.2rem",fontWeight:800,color:T.dark,marginBottom:10,letterSpacing:"-.02em"}}>Haben Sie schon eine Website?</h2><p style={{fontSize:".88rem",color:T.textSub,lineHeight:1.65,marginBottom:24}}>Wir importieren Ihre Daten automatisch – Sie pruefen und korrigieren nur noch.</p><Field label="Ihre aktuelle Website-URL" value={importUrl} onChange={setImportUrl} placeholder="https://ihre-website.at" hint="Wir extrahieren Firmendaten, Leistungen & Kontakt automatisch"/>{importLoading?(<div style={{display:"flex",alignItems:"center",gap:12,padding:"16px 20px",background:T.accentLight,borderRadius:T.rSm,border:`1px solid rgba(37,99,235,.12)`,marginBottom:12}}><div style={{width:18,height:18,borderRadius:"50%",border:`2.5px solid ${T.accent}`,borderTopColor:"transparent",animation:"spin 1s linear infinite",flexShrink:0}}/><span style={{fontSize:".88rem",color:T.accent,fontWeight:600}}>Daten werden importiert...</span></div>):(<button onClick={simulateImport} style={{width:"100%",padding:"14px",border:"none",borderRadius:T.rSm,background:T.dark,color:"#fff",cursor:"pointer",fontSize:".92rem",fontWeight:700,fontFamily:T.font,boxShadow:"0 2px 12px rgba(0,0,0,.12)",marginBottom:12}}>Daten importieren &rarr;</button>)}</div>):pages[step]}</div>

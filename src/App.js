...
const BRANCHEN = [
  ...
  { value:"kosmetik",        gruppe:"kosmetik", label:"Kosmetikstudio",                    leistungen:["Gesichtsbehandlungen","Koerperpflege & Peeling","Waxing & Haarentfernung","Anti-Aging-Behandlungen","Augenbrauen & Wimpern","Beratung & Pflegeroutine"],stil:"modern" },
  ...
  { value:"fusspflege",      gruppe:"kosmetik", label:"Fuss- & Koerperpflege",             leistungen:["Medizinische Fusspflege","Nagelpflege & Korrektur","Hornhautentfernung","Fusspeeling & Entspannungsbad","Gels & Lack","Beratung"],stil:"professional" },
  ...
  { value:"sonstige_kosmetik",gruppe:"kosmetik",label:"Sonstige Kosmetik & Koerperpflege", leistungen:[],stil:"modern" },
];
const BERUFSGRUPPEN=[
      ...
      <div className="lp-hero-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:72,alignItems:"center",padding:"60px 0 80px"}}>
        <div>
          <div style={{display:"inline-flex",alignItems:"center",gap:7,border:"1px solid rgba(37,99,235,.18)",color:T.accent,padding:"5px 13px",borderRadius:6,fontSize:".72rem",fontWeight:700,letterSpacing:".1em",textTransform:"uppercase",marginBottom:28}}>
            ...
            Fuer Kleinbetriebe in Oesterreich
          </div>
  ...
  <Sec id="problem" alt>
    <div className="lp-problem-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:80,alignItems:"start"}}>
      <div>
        ...
        <H2 s={{maxWidth:420}}>Keine gute Option fuer Kleinbetriebe.</H2>
...
const up=useCallback(k=>v=>setData(d=>({...d,[k]:v})),[setData]);const go=n=>{setStep(n);if(ref.current)ref.current.scrollTop=0};const pct=((step+1)/STEPS.length)*100;const selectedBranche=BRANCHEN.find(b=>b.value===data.branche);const brancheLeistungen=selectedBranche?selectedBranche.leistungen:[];const onBrancheChange=val=>{const b=BRANCHEN.find(x=>x.value===val);setData(d=>({...d,branche:val,brancheLabel:b?b.label:"",brancheCustom:"",stil:b?b.stil:d.stil,leistungen:[],extraLeistung:""}))};const onBerufsgruppChange=val=>{setData(d=>({...d,berufsgruppe:val,branche:"",brancheLabel:"",brancheCustom:"",leistungen:[],extraLeistung:""}));};
  ...
  const pages=[<>
    <div style={{marginBottom:4}}>
      <div style={{fontSize:".78rem",fontWeight:700,color:T.textSub,marginBottom:10}}>Berufsgruppe</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {BERUFSGRUPPEN.map(g=>{const on=data.berufsgruppe===g.value;return(<button key={g.value} onClick={()=>onBerufsgruppChange(g.value)} style={{padding:"14px 12px",border:on?`2px solid ${T.accent}`:`1.5px solid ${T.bg3}`,borderRadius:T.r,background:on?T.accentLight:"#fff",cursor:"pointer",textAlign:"left",fontFamily:T.font,transition:"all .2s",boxShadow:on?`0 0 0 3px ${T.accentGlow}`:"none"}}>
          <div style={{fontSize:"1.2rem",marginBottom:4}}>{g.icon}</div>
          <div style={{fontWeight:700,fontSize:".85rem",color:T.dark,marginBottom:2}}>{g.label}</div>
          <div style={{fontSize:".7rem",color:T.textMuted,lineHeight:1.4}}>{g.desc}</div>
        </button>);})}
      </div>
      <div style={{marginTop:8,fontSize:".7rem",color:T.textMuted,textAlign:"center"}}>Weitere Berufsgruppen kommen bald &middot; <span style={{background:T.bg3,padding:"1px 7px",borderRadius:4,fontWeight:600,fontSize:".65rem"}}>Coming soon</span></div>
    </div>
    <Field label="Firmenname" value={data.firmenname} onChange={up("firmenname")} placeholder="z.B. Elektro Mueller GmbH"/>
    <Dropdown label="Branche" value={data.branche} onChange={onBrancheChange} options={filteredBranchen} placeholder="Branche waehlen" hint="Leistungen und Stil werden vorgeschlagen"/>

...
function Questionnaire({data,setData,onComplete,onBack}){
  ...
  const doImport=async()=>{if(!importUrl.trim())return;setImportLoading(true);setImportErr("");setImportPhase("Website wird gelesen...");
    ...
    }catch(e){timers.forEach(clearTimeout);setImportPhase("");setImportLoading(false);if(e.name==="AbortError")setImportErr("Der Import hat zu lange gedauert. Die Website ist möglicherweise nicht erreichbar oder zu komplex. Sie können die Daten manuell eingeben oder uns den Import unter support@siteready.at überlassen.");else setImportErr("Verbindungsfehler — bitte versuchen Sie es erneut oder schreiben Sie uns an support@siteready.at");}};

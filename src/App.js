...
function SuccessPage({data,onBack}){
  ...
  const handleOrder=async()=>{
    ...
    const{error}=await supabase.from("orders").insert({
      ...
      website_ziel:data.websiteZiel||null
    });
        ...
        <div className="sp-card" style={{background:"#fff",borderRadius:T.r,padding:"28px 32px",border:`2px solid ${T.bg3}`,boxShadow:T.sh2}}>
          <div className="sp-price-row" style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",marginBottom:20,paddingBottom:20,borderBottom:`1px solid ${T.bg3}`,flexWrap:"wrap",gap:12}}>
            ...
            {saved
              ...
              :<button onClick={handleOrder} disabled={saving||!regOk} style={{padding:"12px 24px",border:"none",borderRadius:T.rSm,background:saving?T.bg3:!regOk?"#cbd5e1":T.dark,color:"#fff",fontSize:".88rem",fontWeight:700,fontFamily:T.font,cursor:saving?"wait":!regOk?"not-allowed":"pointer",whiteSpace:"nowrap",transition:"background .2s"}}>
...
<div style={{marginTop:8,padding:"12px 14px",background:T.accentLight,borderRadius:T.rSm,border:`1px solid rgba(143,163,184,.15)`}}><div style={{fontSize:".78rem",color:T.accent,lineHeight:1.65}}>Diese Angaben werden automatisch in Ihr Impressum eingebaut (ECG-konform).<br/>Unterstützte Rechtsformen: e.U., Einzelunternehmen, GmbH, OG, KG, AG, Verein, GesbR. Bei anderen Rechtsformen bitte vorab Kontakt aufnehmen.</div></div></>);})()}</>,<><Dropdown label="Was ist das Hauptziel Ihrer Website?" value={data.websiteZiel} onChange={up("websiteZiel")} options={[{value:"leads",label:"Kundenanfragen & Aufträge gewinnen"},{value:"praesenz",label:"Professionelle Online-Präsenz zeigen"},{value:"buchung",label:"Online-Terminbuchung & Reservierungen"},{value:"info",label:"Kunden informieren (Leistungen, Öffnungszeiten)"}]} placeholder="Ziel wählen (optional)" hint="Hilft uns, Texte und Buttons optimal zu gestalten"/>
<div style={{marginTop:8,marginBottom:20,paddingTop:16,borderTop:`1px solid ${T.bg3}`}}><p style={{fontSize:".88rem",color:T.textSub,margin:"0 0 6px",lineHeight:1.6}}>Basierend auf Ihrer Branche empfehlen wir:</p><p style={{fontSize:"1rem",fontWeight:700,color:T.dark,margin:"0 0 16px"}}>{STYLES_MAP[data.stil]?.label||"Professionell"}</p></div><StylePicker value={data.stil} onChange={up("stil")}/><div style={{marginTop:20,padding:"14px 16px",background:T.accentLight,borderRadius:T.rSm,border:`1px solid rgba(143,163,184,.15)`}}><div style={{fontSize:".78rem",fontWeight:700,color:T.accent,marginBottom:6}}>Nach dem Kauf – Self-Service-Portal</div><div style={{fontSize:".82rem",color:T.textSub,lineHeight:1.7}}>Logo hochladen &middot; Eigene Fotos hochladen &middot; Custom Domain verbinden – alles selbst, jederzeit.</div></div></>];

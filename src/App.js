...
function Portal({session,onLogout}){
  ...
  const TABS=[
    ...
    {id:"marketing",label:"Marketing",icon:`<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m22 8-6 4 6 4V8Z"/><rect x="2" y="6" width="14" height="12" rx="2"/></svg>`},
      ...
      {tab==="marketing"&&(<div style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        <div style={{background:"#fff",borderRadius:T.r,padding:"24px 28px",border:`1px solid ${T.bg3}`,boxShadow:T.sh2}}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            <input value={gUrl} onChange={e=>setGUrl(e.target.value)} placeholder="https://maps.google.com/maps/place/..." style={{flex:1,padding:"10px 14px",border:`1.5px solid ${T.bg3}`,borderRadius:T.rSm,fontSize:".82rem",fontFamily:T.font,color:T.dark,outline:"none",background:T.bg}}/>
            <button onClick={async()=>{if(!gUrl.trim())return;try{await supabase.from("orders").update({google_maps_url:gUrl.trim()}).eq("id",order.id);setGSaved(true);setToastMsg("Google-Link gespeichert!");}catch(e){/*ignore*/}}} style={{padding:"10px 18px",background:T.dark,color:"#fff",border:"none",borderRadius:T.rSm,fontSize:".8rem",fontWeight:700,cursor:"pointer",fontFamily:T.font,flexShrink:0}}>Speichern</button>
          </div>
          {gSaved&&gUrl.trim()&&(<div style={{display:"flex",flexDirection:"column",gap:8}}>
            <button onClick={()=>{const text=`Wir freuen uns über Ihre Bewertung: ${gUrl.trim()}`;window.open(`https://wa.me/?text=${encodeURIComponent(text)}`,"_blank");}} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",background:"#dcfce7",border:"1.5px solid #bbf7d0",borderRadius:T.rSm,cursor:"pointer",fontFamily:T.font,fontSize:".82rem",fontWeight:600,color:"#16a34a",textAlign:"left"}}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#16a34a"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"/></svg>
              Bewertungslink per WhatsApp senden
            </button>
            <button onClick={()=>{navigator.clipboard.writeText(gUrl.trim());setToastMsg("Bewertungslink kopiert!");}} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 16px",background:T.bg,border:`1.5px solid ${T.bg3}`,borderRadius:T.rSm,cursor:"pointer",fontFamily:T.font,fontSize:".82rem",fontWeight:600,color:T.textSub,textAlign:"left"}}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
        ...
        <div style={{background:"#fff",borderRadius:T.r,padding:"24px 28px",border:`1px solid ${T.bg3}`,boxShadow:T.sh2}}>
          ...
          <p style={{fontSize:".82rem",color:T.textSub,lineHeight:1.6,margin:"0 0 16px"}}>Professioneller One-Pager zum Ausdrucken — für Kunden, Pinnwände und als E-Mail-Beilage.</p>
          <button onClick={()=>{const websiteUrl=`https://sitereadyprototype.pages.dev/s/${sub}`;const qr=`https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=${encodeURIComponent(websiteUrl)}`;const leistungen=order.leistungen||[];const half=Math.ceil(leistungen.length/2);const col1=leistungen.slice(0,half);const col2=leistungen.slice(half);const logoHtml=assetUrls.logo?`<img src="${assetUrls.logo}" alt="Logo" style="height:60px;max-width:240px;object-fit:contain;"/>`:`<div style="font-size:28px;font-weight:800;color:#0f172a;">${order.firmenname||""}</div>`;const w=window.open("","_blank");w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${order.firmenname||"Firmen-Flyer"}</title><style>@page{size:A4;margin:20mm;}body{font-family:'Segoe UI',system-ui,sans-serif;color:#1a1a1a;margin:0;padding:0;line-height:1.5;}*{box-sizing:border-box;}.page{max-width:170mm;margin:0 auto;}.header{margin-bottom:24px;}.name{font-size:26px;font-weight:800;margin:16px 0 4px;color:#0f172a;}.desc{font-size:15px;color:#475569;margin:0 0 24px;}.section-title{font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#64748b;margin:0 0 12px;border-bottom:2px solid #e2e8f0;padding-bottom:6px;}.leistungen{display:grid;grid-template-columns:1fr 1fr;gap:6px 24px;margin-bottom:28px;}.leistung{font-size:14px;padding:3px 0;}.check{color:#16a34a;margin-right:6px;font-weight:700;}.contact{margin-bottom:28px;}.contact-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px 24px;}.contact-item{font-size:13px;}.contact-label{font-weight:700;color:#64748b;font-size:12px;text-transform:uppercase;letter-spacing:.05em;}.footer{display:flex;align-items:flex-end;justify-content:space-between;margin-top:32px;padding-top:16px;border-top:2px solid #e2e8f0;}.qr-section{display:flex;align-items:center;gap:16px;}.qr-text{font-size:13px;color:#475569;}.qr-text strong{display:block;color:#0f172a;font-size:14px;margin-bottom:2px;}.branding{font-size:11px;color:#94a3b8;text-align:right;}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}</style></head><body><div class="page"><div class="header">${logoHtml}</div><div class="name">${order.firmenname||""}</div><p class="desc">${order.kurzbeschreibung||""}</p>${leistungen.length?`<div class="section-title">Unsere Leistungen</div><div class="leistungen">${leistungen.map(l=>`<div class="leistung"><span class="check">&#10003;</span>${l}</div>`).join("")}</div>`:""}<div class="section-title">Kontakt</div><div class="contact"><div class="contact-grid">${order.adresse?`<div class="contact-item"><div class="contact-label">Adresse</div>${order.adresse}${order.plz||order.ort?", "+[order.plz,order.ort].filter(Boolean).join(" "):""}</div>`:""} ${order.telefon?`<div class="contact-item"><div class="contact-label">Telefon</div>${order.telefon}</div>`:""} ${order.email?`<div class="contact-item"><div class="contact-label">E-Mail</div>${order.email}</div>`:""} ${order.oeffnungszeiten?`<div class="contact-item"><div class="contact-label">Öffnungszeiten</div>${order.oeffnungszeiten}</div>`:""} ${order.einsatzgebiet?`<div class="contact-item"><div class="contact-label">Einsatzgebiet</div>${order.einsatzgebiet}</div>`:""}</div></div><div class="footer"><div class="qr-section"><img src="${qr}" alt="QR" style="width:80px;height:80px;"/><div class="qr-text"><strong>Besuchen Sie uns online</strong>${sub}.siteready.at</div></div><div class="branding">Erstellt mit SiteReady</div></div></div></body></html>`);w.document.close();w.onload=()=>w.print();}} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 20px",background:T.dark,color:"#fff",border:"none",borderRadius:T.rSm,cursor:"pointer",fontFamily:T.font,fontSize:".82rem",fontWeight:700}}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9V2h12v7"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>

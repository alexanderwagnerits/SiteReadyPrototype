            ...
            {(order.leistungen||[]).length>0&&<div style={{marginBottom:20}}>
              ...
              {(order.leistungen||[]).map((l,i,arr)=>(
                <div key={l} style={{marginBottom:8,background:"#fff",border:`1px solid ${T.bg3}`,borderLeft:`3px solid ${T.accent}`,borderRadius:T.rSm,overflow:"hidden"}}>
                  ...
                  <input value={(order.leistungen_preise||{})[l]||""} onChange={e=>{const m={...(order.leistungen_preise||{})};m[l]=e.target.value;upOrder("leistungen_preise")(m);}} onBlur={async()=>{if(supabase&&order?.id)await supabase.from("orders").update({leistungen_preise:order.leistungen_preise}).eq("id",order.id);}} placeholder={"Preis (z.B. ab \u20ac80) \u2013 optional"} style={{width:"100%",padding:"8px 12px",border:"none",borderBottom:`1px solid ${T.bg3}`,fontSize:".82rem",fontFamily:T.font,background:"#fff",color:T.dark,outline:"none",boxSizing:"border-box"}}/>

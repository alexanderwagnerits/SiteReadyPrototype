...
const STYLES_MAP={
  ...
  modern:{label:"Modern",desc:"Frisch, freundlich, mit Akzenten",primary:"#111111",accent:"#6366f1",accentSoft:"rgba(99,102,241,0.07)",bg:"#fafafa",cardBg:"#fff",text:"#111111",textMuted:"#6b7280",textLight:"#9ca3af",borderColor:"#f0f0f0",font:"'DM Sans',system-ui,sans-serif",radius:"12px",radiusLg:"16px",heroGradient:"linear-gradient(135deg,#111111 0%,#1f1f1f 50%,#2d2d2d 100%)",heroOverlay:"radial-gradient(ellipse at 80% 30%,rgba(99,102,241,0.15) 0%,transparent 50%)",shadow:"0 2px 8px rgba(0,0,0,0.06)",badgeBg:"#eef2ff",badgeText:"#4f46e5",btnRadius:"100px",cardBorder:false,cardShadow:true,badgeRadius:"100px",sectionDivider:false,spacing:"airy"},
  elegant:{label:"Elegant",desc:"Hochwertig, ruhig, Premium",primary:"#1c1917",accent:"#78716c",accentSoft:"rgba(120,113,108,0.06)",bg:"#fafaf9",cardBg:"#fff",text:"#1c1917",textMuted:"#78716c",textLight:"#a8a29e",borderColor:"#e7e5e4",font:"'Inter',system-ui,sans-serif",radius:"2px",radiusLg:"4px",heroGradient:"linear-gradient(160deg,#1c1917 0%,#292524 50%,#44403c 100%)",heroOverlay:"none",shadow:"none",badgeBg:"#f5f5f4",badgeText:"#57534e",btnRadius:"2px",cardBorder:true,cardShadow:false,badgeRadius:"2px",sectionDivider:true,spacing:"airy"},
...
/* ═══ PREVIEW (unchanged) ═══ */
const CUSTOM_FONTS={dm_sans:"'DM Sans',sans-serif",inter:"'Inter',sans-serif",outfit:"'Outfit',sans-serif",source_serif:"'Source Serif 4',Georgia,serif",playfair:"'Playfair Display',Georgia,serif"};
function buildCustomStyle(d){const c=d.customColor||"#2563eb";const f=CUSTOM_FONTS[d.customFont]||CUSTOM_FONTS.dm_sans;const r=parseInt(c.slice(1,3),16),g=parseInt(c.slice(3,5),16),b=parseInt(c.slice(5,7),16);return{...STYLES_MAP.custom,primary:c,accent:c,accentSoft:`rgba(${r},${g},${b},0.07)`,font:f,heroGradient:`linear-gradient(160deg,${c} 0%,rgba(${r},${g},${b},0.85) 50%,rgba(${r},${g},${b},0.7) 100%)`,badgeBg:`rgba(${r},${g},${b},0.08)`,badgeText:c};}
...
<div>
<label style={{display:"block",marginBottom:7,fontSize:".8rem",fontWeight:700,color:T.textSub,letterSpacing:".03em"}}>Schriftart</label>
<div style={{display:"flex",flexDirection:"column",gap:6}}>
{[{value:"dm_sans",label:"DM Sans",family:"'DM Sans',sans-serif",desc:"Modern & freundlich"},{value:"inter",label:"Inter",family:"'Inter',sans-serif",desc:"Klar & professionell"},{value:"outfit",label:"Outfit",family:"'Outfit',sans-serif",desc:"Frisch & zeitgemäß"},{value:"source_serif",label:"Source Serif",family:"'Source Serif 4',Georgia,serif",desc:"Klassisch & elegant"},{value:"playfair",label:"Playfair Display",family:"'Playfair Display',Georgia,serif",desc:"Hochwertig & stilvoll"}].map(f=>{const on=data.customFont===f.value;return<button key={f.value} onClick={()=>up("customFont")(f.value)} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",border:on?`2px solid ${T.accent}`:`1.5px solid ${T.bg3}`,borderRadius:T.rSm,background:on?T.accentLight:"#fff",cursor:"pointer",fontFamily:T.font,transition:"all .15s"}}>
<div><span style={{fontFamily:f.family,fontSize:".95rem",fontWeight:600,color:T.dark}}>{f.label}</span><span style={{fontSize:".75rem",color:T.textMuted,marginLeft:8}}>{f.desc}</span></div>
{on&&<span style={{color:T.accent,fontWeight:700,fontSize:".85rem"}}>{"\u2713"}</span>}
</button>})}
</div>
</div>
</div>}

...
function Questionnaire({data,setData,onComplete,onBack}){
  ...
  const doImport=async()=>{if(!importUrl.trim())return;setImportLoading(true);setImportErr("");setImportPhase("Website wird gelesen...");
    ...
    try{const ctrl=new AbortController();const timeout=setTimeout(()=>ctrl.abort(),55000);
      ...
      <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:"36px 20px",overflowY:"auto"}}>
        <div style={{maxWidth:520,width:"100%",textAlign:"center"}}>
          ...
          <div className="q-content-card" style={{textAlign:"left",marginBottom:24}}>
            ...
            {importLoading&&<div style={{marginTop:10,display:"flex",alignItems:"center",gap:8,padding:"10px 14px",background:T.bg,borderRadius:T.rSm,border:`1px solid ${T.bg3}`}}>
              <div style={{width:14,height:14,borderRadius:"50%",border:`2px solid ${T.accent}`,borderTopColor:"transparent",animation:"spin 1s linear infinite",flexShrink:0}}/>
              <span style={{fontSize:".85rem",color:T.textSub,fontWeight:500}}>{importPhase}</span>
            </div>}
            {importErr&&<div style={{marginTop:8,padding:"10px 14px",background:"#fef2f2",borderRadius:T.rSm,border:"1px solid #fecaca",fontSize:".85rem",color:T.red,lineHeight:1.5}}>{importErr}</div>}
...
function Portal({session,onLogout}){
  ...
  const saveSection=async(section)=>{
    ...
    const fields={
      ...
      kontakt:{adresse:order.adresse,plz:order.plz,ort:order.ort,telefon:order.telefon,oeffnungszeiten:order.oeffnungszeiten,oeffnungszeiten_custom:order.oeffnungszeiten_custom,kontakt_formular:order.kontakt_formular||null,gut_zu_wissen:order.gut_zu_wissen||null},
  ...
  const astItems=order?[
    ...
    {label:"Kundenbewertungen",done:!!(order.bewertungen?.some(b=>b.text)),pts:0,page:"ueberuns",optional:true},
